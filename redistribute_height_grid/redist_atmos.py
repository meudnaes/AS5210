...
class REDIST_ATMOS:
    ...
    def tau_nu(self, data, idx=0, idy=0):
        ...
        height : n-D array
            height scale
        """
        height = data.atmos.height_scale[idx, idy].dropna('height')
        tau_nu = np.zeros((data.ray.chi[idx, idy].dropna('height').shape))
        for i in range(tau_nu.shape[1]):
            tau_nu[:, i] = cumtrapz(data.ray.chi[idx, idy, :, i].dropna('height'),
                                    axis= -1, x=-height,
                                    initial=1e-20)
    ...
    def tau_redist(self, tau_min=-3, tau_max=2):
        """
        ...
        f_tau = interp1d(self.height[upper_limit:lower_limit], tau_sum, kind="cubic")
        x_tmp = np.linspace(self.height[upper_limit], self.height[lower_limit-1], int(1e5))

        tau_height = np.zeros(len(self.height))

        max_diff = tau_sum.max() - tau_sum.min()
        point_diff = max_diff/(len(self.height)-1)

        tau_height[0] = self.height[upper_limit]

        idz = 1
        for i in range(len(x_tmp)-1):
            if f_tau(x_tmp[i]) >= idz*point_diff:
                tau_height[idz] = x_tmp[i]
                idz += 1
        tau_height[-1] = self.height[lower_limit-1]

        return tau_height

    def brightness_temperature(self, nu, I_nu):
        """
        Method to calculate brightness temperature

        Parameters
        ----------
        I_nu : n-D array (float)
            Specific intensity, si units
        ...
        T_b = h*nu/k_B*np.log(1 + 2*h*nu**3/(I_nu*c**2))**(-1)
